...
// Convert frontend period selector → backend from/to params
const periodToRange = (period) => {
  const toStr   = new Date().toISOString().slice(0, 10);
  if (period === "today")  return { from: toStr, to: toStr };
  if (period === "7d")     return {};  // backend default is 7d anyway
...
// Backend returns 'available' for online drivers; treat same as 'online'
const STATUS_CFG = {
  available: { dot:"#22c55e", bg:"rgba(34,197,94,0.1)",  border:"rgba(34,197,94,0.28)",  label:"Online",  map:"#22c55e" },
...
function DriversListSection() {
  ...
  // filter status: 'all' | 'available' | 'on_ride' | 'offline'
  ...
  const loadSumm = useCallback(async () => {
    setSLoading(true);
    try {
      const r = await getDMDriversSummary();
      // Response: { totalDrivers, online, onRide, offline, unverified, deactivated, neverOnline }
  ...
  const loadDrivers = useCallback(async () => {
    ...
    try {
      ...
      const p = { limit: LIMIT, offset, ...dateRange };
      if (search)           p.search      = search;
      if (status !== "all") p.status      = status;  // 'available' | 'on_ride' | 'offline'
      if (vtype  !== "all") p.vehicleType = vtype;   // camelCase — backend expects vehicleType

      const r = await getDMDrivers(p);
      const d = r.data?.data || r.data || {};
      // Response shape: { total, limit, offset, from, to, sort, items: [...] }
      setDrivers(d.items || []);
      setPag({ total: d.total || 0 });
    } catch(e) {
      showToast(e?.response?.data?.message || "Failed to load drivers");
      setDrivers([]);
    } finally { setLoading(false); }
  }, [search, status, vtype, period, offset]);

  useEffect(() => { loadSumm(); }, [loadSumm]);
  useEffect(() => { setOffset(0); }, [search, status, vtype, period]);
  ...
  return (
    <div>
      ...
      {/* Summary chips — uses actual backend field names */}
      <div style={{ display:"flex",gap:10,flexWrap:"wrap",marginBottom:20 }}>
        <SummChip label="Total Drivers"  value={fmtNum(summary?.totalDrivers)}   loading={sLoading}/>
        <SummChip label="Online"         value={fmtNum(summary?.online)}          color="#22c55e" loading={sLoading}/>
        <SummChip label="On Ride"        value={fmtNum(summary?.onRide)}          color="#60a5fa" loading={sLoading}/>
        <SummChip label="Offline"        value={fmtNum(summary?.offline)}         color="#4b5563" loading={sLoading}/>
        <SummChip label="Unverified"     value={fmtNum(summary?.unverified)}      color="#F59E0B" loading={sLoading}/>
        <SummChip label="Never Online"   value={fmtNum(summary?.neverOnline)}     color="#6b7280" loading={sLoading}/>
      </div>
      ...
      <div style={{ display:"flex",gap:8,flexWrap:"wrap",marginBottom:16,alignItems:"center" }}>
        <div style={{ position:"relative",flex:"2 1 200px",minWidth:160 }}>
          ...
          <input className="dm-inp" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search name / phone…" style={{ paddingLeft:30,width:"100%" }}/>
        </div>
        {/* Status filter: send 'available' for Online to match backend */}
        <select className="dm-inp" value={status} onChange={e=>setStatus(e.target.value)} style={{ flex:"0 0 130px",cursor:"pointer" }}>
        ...
        <select className="dm-inp" value={period} onChange={e=>setPeriod(e.target.value)} style={{ flex:"0 0 118px",cursor:"pointer" }}>
        ...
        {(search || status !== "all" || vtype !== "all") && (
          <button onClick={()=>{ setSearch(""); setStatus("all"); setVtype("all"); }} style={{ height:38,padding:"0 12px",background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.28)",borderRadius:10,color:"#f87171",cursor:"pointer",display:"flex",alignItems:"center",gap:5,fontSize:13,fontFamily:"Outfit,sans-serif" }}>
            <X size={12}/> Clear
          </button>
        )}
      </div>
          ...
          <table style={{ width:"100%",borderCollapse:"collapse" }}>
            ...
            <tbody>
              {loading ? Array.from({length:8}).map((_,i)=>(
                ...
                // d.driverId — backend uses driverId not id
                // d.vehicle.types — array
                // d.range.ridesCompleted, d.range.grossFare, d.range.acceptanceRate
                // d.city?.name — city is object { id, name }
                const vt = d.vehicle?.types || [];
                return (
                  <tr key={d.driverId} className="dm-tr" onClick={()=>navigate(`/driver-metrics/${d.driverId}`)}>
                    <td style={tdStyle}>
                      <div style={{ fontWeight:600,color:"#fff",marginBottom:2 }}>{d.name || "—"}</div>
                      <div style={{ fontSize:11,color:"rgba(255,255,255,0.38)" }}>{d.phone || "—"}</div>
                    </td>
                    <td style={tdStyle}>
                    ...
                    <td style={{ ...tdStyle,fontVariantNumeric:"tabular-nums" }}>
                      {fmtNum(d.range?.ridesCompleted)}
                    </td>
                    <td style={{ ...tdStyle,color:"#D4AF37",fontWeight:600,fontVariantNumeric:"tabular-nums" }}>
                      {fmtRupee(d.range?.grossFare)}
                    </td>
                    ...
                    <td style={tdStyle}>
                      <button onClick={e=>{ e.stopPropagation(); navigate(`/driver-metrics/${d.driverId}`); }} style={{ background:"rgba(212,175,55,0.08)",border:"1px solid rgba(212,175,55,0.22)",borderRadius:8,padding:"4px 10px",color:"#D4AF37",cursor:"pointer",display:"inline-flex",alignItems:"center",gap:4,fontSize:11,fontFamily:"Outfit,sans-serif" }}>
...
function LiveMapSection() {
  ...
  // Summary field path: response.totals.totalOnline etc.
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex",gap:10,flexWrap:"wrap",marginBottom:16 }}>
        <SummChip label="Total Online"   value={fmtNum(totals.totalOnline)}          color="#22c55e" loading={sLoading}/>
        <SummChip label="Available"      value={fmtNum(totals.available)}            color="#4ade80" loading={sLoading}/>
        <SummChip label="On Ride"        value={fmtNum(totals.onRide)}               color="#60a5fa" loading={sLoading}/>
        <SummChip label="Stale Pings"    value={fmtNum(totals.stale)}                color="#F59E0B" loading={sLoading}/>
        <SummChip label="No Location"    value={fmtNum(totals.withoutLocation)}      color="#f87171" loading={sLoading}/>
      ...
      <div style={{ display:"flex",gap:14,marginBottom:12,flexWrap:"wrap" }}>
        {[{c:"#22c55e",l:"Available"},{c:"#60a5fa",l:"On Ride"},{c:"#4b5563",l:"Offline"},{c:"#F59E0B",l:"Ghost (stale ping)"}].map(({c,l})=>(
      ...
      <div className="dm-card" style={{ height:520,overflow:"hidden",borderRadius:18 }}>
        {loading && drivers.length === 0 ? (
          ...
          <MapContainer center={center} zoom={5} style={{ height:"100%",width:"100%",background:"#020d26" }}>
            <TileLayer
              url="https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png"
              attribution='&copy; <a href="https://carto.com/">CARTO</a>'
            />
            {drivers.map(d => {
              // Backend: location.latitude / location.longitude (not lat/lng)
              const lat = d.location?.latitude  ?? d.location?.lat;
              const lng = d.location?.longitude ?? d.location?.lng;
              if (!lat || !lng) return null;
              const col = getMarkerColor(d);
              const vt  = d.vehicle?.types || [];
              return (
                <CircleMarker
                  key={d.driverId}
                ...
                >
                  <Popup>
                    <div style={{ minWidth:180 }}>
                      <p style={{ margin:"0 0 6px",fontWeight:700,fontSize:14,color:"#fff" }}>{d.name || "Driver #" + d.driverId}</p>
                      <div style={{ display:"flex",gap:6,flexWrap:"wrap",marginBottom:6 }}>
                        <StatusPill status={d.status}/>
                        <ConnectionBadge conn={d.connection}/>
                      </div>
                      {vt.length > 0 && <VtBadge types={vt}/>}
                      {d.city?.name && <p style={{ margin:"6px 0 0",fontSize:11,color:"rgba(255,255,255,0.5)" }}>📍 {d.city.name}</p>}
                      <button
                        onClick={()=>navigate(`/driver-metrics/${d.driverId}`)}
      ...
      <p style={{ marginTop:10,fontSize:11,color:"rgba(255,255,255,0.25)",fontFamily:"Outfit,sans-serif",textAlign:"right" }}>
        {drivers.length} driver{drivers.length !== 1 ? "s" : ""} with location · auto-refreshes every 15s
      </p>
    </div>
  );
}
...
function FleetAnalyticsSection() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      const [lbRes, fsRes, bcRes] = await Promise.all([
        getDMLeaderboard(dateRange),
        getDMFleetSummary(dateRange),
        getDMBreakCompliance(dateRange),
      ]);
      setData({
        leaderboard: lbRes.data?.data?.drivers || lbRes.data?.data?.items || lbRes.data?.data || [],
        fleet:       fsRes.data?.data || fsRes.data || null,
        compliance:  bcRes.data?.data?.drivers || bcRes.data?.data?.items || bcRes.data?.data || [],
      });
    } catch(e) {
  ...
  const vtDist  = data.fleet?.vehicleTypeDistribution || data.fleet?.vehicle_type_distribution || [];
  const vtPie   = vtDist.map((v,i) => ({ name: v.vehicleType||v.vehicle_type||"Other", value: Number(v.count||v.drivers||0), color: PIE_COLORS[i%PIE_COLORS.length] }));
  const dailyD  = data.fleet?.daily || [];
        ...
        <div className="dm-card" style={{ overflow:"hidden" }}>
          <div style={{ overflowX:"auto" }}>
            <table style={{ width:"100%",borderCollapse:"collapse" }}>
              <thead>
                <tr style={{ background:"rgba(212,175,55,0.05)",borderBottom:"1px solid rgba(212,175,55,0.1)" }}>
                  {["#","Driver","Rides","Acceptance","Completion","Earnings","Rating"].map(h=><th key={h} style={thStyle}>{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {loading ? Array.from({length:6}).map((_,i)=>(
                  ...
                  <tr><td colSpan={7} style={{ ...tdStyle,textAlign:"center",padding:"40px",color:"rgba(255,255,255,0.25)",fontStyle:"italic" }}>No leaderboard data for this period</td></tr>
                ) : data.leaderboard.map((d,i) => (
                  <tr key={d.driverId||d.id||i} className="dm-tr-plain">
                    ...
                    <td style={{ ...tdStyle,fontVariantNumeric:"tabular-nums" }}>{fmtNum(d.ridesCompleted||d.rides||d.total_rides)}</td>
                    <td style={{ ...tdStyle,color:(d.acceptanceRate||d.acceptance_rate)>=80?"#22c55e":(d.acceptanceRate||d.acceptance_rate)>=60?"#D4AF37":"#f87171" }}>{fmtRate(d.acceptanceRate||d.acceptance_rate)}</td>
                    <td style={{ ...tdStyle,color:(d.completionRate||d.completion_rate)>=90?"#22c55e":(d.completionRate||d.completion_rate)>=75?"#D4AF37":"#f87171" }}>{fmtRate(d.completionRate||d.completion_rate)}</td>
                    <td style={{ ...tdStyle,color:"#D4AF37",fontWeight:600,fontVariantNumeric:"tabular-nums" }}>{fmtRupee(d.grossFare||d.earnings||d.total_earnings)}</td>
      ...
      {sub === 1 && (
        <div>
          <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:10,marginBottom:20 }}>
            {loading ? Array.from({length:6}).map((_,i)=>(
              <div key={i} className="dm-card" style={{ padding:"16px 18px" }}><Skel w="60%" h={12}/><br/><br/><Skel w="40%" h={22}/></div>
            )) : [
              { label:"Total Drivers",    value:fmtNum(data.fleet?.totalDrivers||data.fleet?.total_drivers),                color:"#fff"    },
              { label:"Total Rides",       value:fmtNum(data.fleet?.totalRides||data.fleet?.total_rides),                   color:"#60a5fa" },
              { label:"Total Revenue",     value:fmtRupee(data.fleet?.totalRevenue||data.fleet?.total_revenue),             color:"#D4AF37" },
              { label:"Avg Acceptance",    value:fmtRate(data.fleet?.avgAcceptanceRate||data.fleet?.avg_acceptance_rate),   color:"#22c55e" },
              { label:"Avg Completion",    value:fmtRate(data.fleet?.avgCompletionRate||data.fleet?.avg_completion_rate),   color:"#22c55e" },
              { label:"Online Hours",      value:data.fleet?.totalOnlineHours!=null?Number(data.fleet.totalOnlineHours).toFixed(1)+"h":"—", color:"#a78bfa" },
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:16 }}>
            <div className="dm-card" style={{ padding:"20px 20px 10px" }}>
              ...
              ) : <div style={{ height:200,display:"flex",alignItems:"center",justifyContent:"center",color:"rgba(255,255,255,0.2)",fontSize:12,fontStyle:"italic" }}>No data</div>}
            </div>
            <div className="dm-card" style={{ padding:"20px 20px 10px" }}>
              ...
              {dailyD.length > 0 ? (
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={dailyD} margin={{ top:4,right:8,left:0,bottom:0 }}>
        ...
        <div className="dm-card" style={{ overflow:"hidden" }}>
          <div style={{ overflowX:"auto" }}>
            <table style={{ width:"100%",borderCollapse:"collapse" }}>
              <thead>
                <tr style={{ background:"rgba(212,175,55,0.05)",borderBottom:"1px solid rgba(212,175,55,0.1)" }}>
                  {["Driver","Scheduled","Taken","Compliance","Break Time","Status"].map(h=><th key={h} style={thStyle}>{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {loading ? Array.from({length:6}).map((_,i)=>(
                  ...
                  const rate = d.complianceRate ?? d.compliance_rate ?? d.rate;
                  const col  = rate===null?"rgba(255,255,255,0.4)":rate>=90?"#22c55e":rate>=70?"#D4AF37":"#f87171";
                  return (
                    <tr key={d.driverId||d.id||i} className="dm-tr-plain">
                      <td style={tdStyle}>
                        <div style={{ fontWeight:600,color:"#fff" }}>{d.name||d.full_name||"—"}</div>
                      ...
                      <td style={{ ...tdStyle,fontVariantNumeric:"tabular-nums" }}>{fmtNum(d.scheduledBreaks||d.scheduled_breaks)}</td>
                      <td style={{ ...tdStyle,fontVariantNumeric:"tabular-nums" }}>{fmtNum(d.breaksTaken||d.breaks_taken)}</td>
                      <td style={{ ...tdStyle,color:col,fontWeight:600 }}>{fmtRate(rate)}</td>
                      <td style={tdStyle}>{fmtMins(d.totalBreakMinutes||d.total_break_minutes)}</td>
                      <td style={tdStyle}>
                        {rate===null?"—":rate>=90
  ...
  return (
    <div style={{ fontFamily:"'Outfit',sans-serif",color:"#fff" }}>
      ...
      <div style={{ marginBottom:24 }}>
        ...
        <h1 style={{ fontFamily:"'Cinzel',serif",fontSize:"clamp(18px,2vw,26px)",fontWeight:700,color:"#fff",margin:0,letterSpacing:-0.3 }}>Driver Metrics</h1>
