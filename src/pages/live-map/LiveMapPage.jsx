...
import { RefreshCw, Bell, X, Send, Car } from 'lucide-react';
...
const vehicleIcon = (type) => {
  const colors = { auto: '#f59e0b', cab: '#3b82f6', premium: '#8b5cf6', bike: '#10b981' };
...
function GroupNotifyModal({ onClose }) {
  const [group, setGroup] = useState('ncr');
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setErr(e.response?.data?.message || 'Notification send karne mein error aaya');
    } finally { setSending(false); }
  };

  const groupLabels = { ncr: 'NCR Drivers', online: 'Online Drivers', all: 'All Drivers' };
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
        ...
        <div className="flex gap-3 p-5 pt-0">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-lg border dark:border-gray-700 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
            Cancel
          </button>
          <button
            onClick={handleSend}
...
export default function LiveMapPage() {
  ...
  const center = drivers.length > 0
    ? [parseFloat(drivers[0].lat), parseFloat(drivers[0].lng)]
  ...
  return (
    <div className="flex flex-col h-full" style={{ minHeight: '100vh' }}>
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-white dark:bg-gray-900 border-b dark:border-gray-800 flex-shrink-0">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            {loading ? 'Loading...' : `${drivers.length} driver${drivers.length !== 1 ? 's' : ''} online`}
            {lastRefresh && (
      ...
      {/* Legend */}
      <div className="px-6 py-2 bg-white dark:bg-gray-900 border-b dark:border-gray-800 flex items-center gap-4 flex-shrink-0">
        {[['auto','#f59e0b'],['cab','#3b82f6'],['premium','#8b5cf6'],['bike','#10b981']].map(([type, color]) => (
        ...
        {loading && drivers.length === 0 ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-gray-50 dark:bg-gray-900">
            <div className="text-center text-gray-500">
              ...
              <p className="text-sm mt-1">Map will auto-refresh every 30 seconds</p>
            </div>
          </div>
        ) : (
          <MapContainer
