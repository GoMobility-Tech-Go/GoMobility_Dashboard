...
const TABS = [
  ...
  { id: "all",         l: "📋 All Drivers"   },
  { id: "chart",       l: "📊 Tier Analytics"},
  { id: "rules",       l: "⚙️ Auto Rules"    },
];

const RANK_MEDALS = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣"];

/** Stat columns shown in each leaderboard card */
const leaderboardStats = (d) => [
  { l: "Rides",    v: d.rides,                            c: "#D4AF37"              },
  { l: "Rating",   v: `${d.rating}★`,                    c: "#34D399"              },
  { l: "Earnings", v: `Rs${(d.earnings / 1000).toFixed(0)}K`, c: "#60A5FA"         },
  { l: "Vehicle",  v: d.vehicle,                          c: "rgba(255,255,255,0.6)"},
];

/** Inline rows inside each tier card */
const tierInfoRows = (conf) => [
  ["Min Rating",      `${conf.minRating}★`  ],
  ["Min Rides",       `${conf.minRides}/month`],
  ["Incentive Bonus", `${conf.bonus} extra`  ],
];

/** Cancel cell color */
const cancelColor = (n) =>
  n > 10 ? "#F87171" : n > 6 ? "#F59E0B" : "#34D399";

// ─────────────────────────────────────────────
// Content (inner, wrapped by ToastProvider)
// ─────────────────────────────────────────────
...
function Content() {
  ...
  const avgRating = (
    drivers.reduce((a, b) => a + parseFloat(b.rating), 0) / drivers.length
  ).toFixed(2);
  ...
  const handleAutoDeactToggle = (v) => {
    setAutoDeact(v);
    toast(`Auto-deactivation ${v ? "enabled" : "disabled"}`, v ? "success" : "error");
  };
  ...
  return (
    <PageWrapper
      title="Driver Performance & Tier System"
      subtitle="Driver tiers, leaderboard, incentives and auto-deactivation rules"
      actions={
        <div style={{ display: "flex", gap: 8 }}>
          <button className="btn-outline btn-sm" onClick={recalcTiers}>
            🔄 Recalculate Tiers
          </button>
          <button
            className="btn-gold btn-sm"
            onClick={() => toast("Reward emails sent to top drivers!", "success")}
          >
            🏆 Send Monthly Rewards
          </button>
        </div>
      }
    >
      ...
      <MiniStatRow
        items={[
          ...
          { label: "Avg Rating",     value: `${avgRating}★`,             icon: "⭐", color: "#D4AF37" },
        ...
        {Object.entries(TIER_CONFIG).map(([tier, conf]) => (
          <Card key={tier} style={{ padding: 22, borderColor: `${conf.color}25` }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
              ...
              <div>
                <div style={{ fontSize: 16, fontWeight: 800, fontFamily: "Cinzel,serif", color: conf.color }}>
                  {tier} Tier
                </div>
                <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginTop: 1 }}>
                  {tierCount(tier)} drivers
                </div>
              </div>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 14 }}>
              {tierInfoRows(conf).map(([label, val], i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
                  <span style={{ color: "rgba(255,255,255,0.45)" }}>{label}</span>
                  <span style={{ color: conf.color, fontWeight: 700 }}>{val}</span>
                </div>
              ))}
            </div>
            ...
            <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 10 }}>
              <div style={{ fontSize: 10, color: "rgba(255,255,255,0.38)", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.6px" }}>
                Perks
              </div>
              {conf.perks.map((p, i) => (
                <div key={i} style={{ fontSize: 11, color: conf.color, marginBottom: 3 }}>
                  ✓ {p}
                </div>
              ))}
            </div>
          ...
          {LEADERBOARD.map((d, i) => {
            ...
            return (
              <Card
                ...
                style={{
                  padding:     "16px 20px",
                  borderColor: i < 3 ? `${conf.color}30` : "rgba(212,175,55,0.12)",
                }}
              >
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  ...
                  <AvatarCell
                    name={d.name}
                    gradient={`linear-gradient(135deg,${conf.color},${conf.color}80)`}
                  />
                ...
                return (
                  <tr key={i}>
                    <td>
                      <AvatarCell
                        name={d.name}
                        gradient={`linear-gradient(135deg,${conf.color},${conf.color}70)`}
                      />
                    </td>
                    <td>
                      <span style={{
                    ...
                    <td style={{ color: cancelColor(d.cancel), fontWeight: 700 }}>{d.cancel}</td>
                    <td>{d.vehicle}</td>
                    <td>
                      <button
                        className="btn-outline btn-xs"
                        onClick={() => toast(`Reward sent to ${d.name}!`, "success")}
                      >
                        Send Reward
                      </button>
                    </td>
      ...
      {/* ══════════════════════════════════════
          TAB: Tier Analytics Chart
      ══════════════════════════════════════ */}
      {tab === "chart" && (
        <Card style={{ padding: 22 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: "rgba(255,255,255,0.85)", marginBottom: 4 }}>
            Tier Distribution Over Months
          </div>
          <div style={{ fontSize: 11, color: "rgba(255,255,255,0.38)", marginBottom: 18 }}>
            Growth of each tier month over month
          </div>
      ...
      {tab === "rules" && (
        <Card style={{ padding: 24 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "rgba(255,255,255,0.85)", marginBottom: 18 }}>
            Auto Deactivation & Tier Rules
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>

            <FormGroup
              label="Max Cancellations Before Auto-Deactivation"
              hint="Driver auto-deactivated if cancellations exceed this limit/month"
            >
              <input
                type="number"
                className="gm-input"
            ...
            <FormGroup
              label="Auto Deactivation"
              hint="Automatically deactivate drivers who exceed cancellation limit"
            >
              <div style={{ paddingTop: 6 }}>
                <Toggle
                  checked={autoDeact}
                  onChange={handleAutoDeactToggle}
                  label={autoDeact ? "Auto-deactivation Active" : "Auto-deactivation Disabled"}
                />
              </div>
            </FormGroup>

            <FormGroup label="Minimum Rides for Gold Tier">
              <input type="number" className="gm-input" defaultValue="300" />
            </FormGroup>

            <FormGroup label="Minimum Rating for Gold Tier">
              <input type="number" className="gm-input" defaultValue="4.5" step="0.1" />
            </FormGroup>

            <FormGroup label="Minimum Rides for Silver Tier">
              <input type="number" className="gm-input" defaultValue="150" />
            </FormGroup>

            <FormGroup label="Minimum Rating for Silver Tier">
              <input type="number" className="gm-input" defaultValue="4.0" step="0.1" />
            </FormGroup>

            <FormGroup label="Gold Bonus Incentive %">
              <input type="number" className="gm-input" defaultValue="25" />
            </FormGroup>

            <FormGroup label="Silver Bonus Incentive %">
              <input type="number" className="gm-input" defaultValue="15" />
            </FormGroup>
          </div>
          ...
          <button
            className="btn-gold"
            style={{ marginTop: 8 }}
            onClick={() => toast("Tier rules saved!", "success")}
          >
            Save Rules
          </button>
        </Card>
